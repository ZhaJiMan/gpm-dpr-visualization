...
class CrossSection:
    ...
    def __init__(self, lon, lat, start, end, npt=100, box=0.2):
        ...
        box : float
            ...
            默认为 0.2 度（DPR 的水平分辨率 5km 约为 0.05 度)。
        ...
        self.line = np.column_stack(
            (np.linspace(start[0], end[0], npt), np.linspace(start[1], end[1], npt))
        )

        # 收集落入每个 box 的数据点的 mask
        half = box / 2
        self.masks = []
    ...
    def __call__(self, data):
        ...
        if data.ndim == 2:
            ...
            raise ValueError("data 是二维或三维数组")
...
def plot_precipitation():
    ...
    for i, (start, end) in enumerate(start_ends):
        ax.plot(*zip(start, end), "o-", lw=1.5, c="k", ms=3, transform=crs)
...
def plot_cross_section(i):
    ...
    for ax in axes:
        ...
        ax.legend(loc="upper right", fontsize="small", fancybox=False, framealpha=1)
    ...
    # 添加标题
    titles = ["DPR Rain Rate", "CSH Latent Heat", "SLH Latent Heat"]
